...
proximal_mu_values = [0.1, 0.3, 0.5, 0.7, 1.0] if algo_choice == 'fedprox' else [0.0]  # Only for FedProx
gamma_min_values = [1.0]  # Fixed as specified
gamma_max_values = [2.0, 3.0, 4.0, 5.0]
scale_factor_values = [2.0, 3.0, 4.0]

# Generate all combinations
param_grid = list(itertools.product(proximal_mu_values, gamma_min_values, gamma_max_values, scale_factor_values))
print(f"Total hyperparameter combinations: {len(param_grid)}")

# Grid search results
grid_results_file = f"gridsearch_results_{model_choice.upper()}_{algo_choice.upper()}.csv"
with open(grid_results_file, 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['Proximal_Mu', 'Gamma_Min', 'Gamma_Max', 'Scale_Factor', 'Best_Balanced_F1', 
                     'Best_Balanced_Acc', 'Best_Round', 'Final_Val_Loss', 'Final_F1', 'Final_Accuracy', 
                     'Final_Balanced_Acc', 'Final_Balanced_F1', 'Final_Balanced_Balanced_Acc'])

best_global_f1 = 0.0
best_global_balanced_acc = 0.0
best_global_params = None
best_global_model_path = f"best_global_model_{model_choice.upper()}_{algo_choice.upper()}.pth"

# Grid search loop
for param_idx, (proximal_mu, gamma_min, gamma_max, scale_factor) in enumerate(param_grid, 1):
    print(f"\n=== Grid Search Iteration {param_idx}/{len(param_grid)}: proximal_mu={proximal_mu}, "
          f"gamma_min={gamma_min}, gamma_max={gamma_max}, scale_factor={scale_factor} ===")
    
    # Initialize class weights and focal loss
    class_weights = compute_class_weights(y_test, scale_factor=scale_factor)
    global focal_criterion
    focal_criterion = AdaptiveFocalLoss(alpha=class_weights, gamma_min=gamma_min, gamma_max=gamma_max)
    
    # Initialize global model
    global_model = initialize_global_model(model_choice, input_dim)
    
    # CSV for round-wise metrics
    csv_file = (f"fl_metrics_{model_choice.upper()}_{algo_choice.upper()}_mu_{proximal_mu}_gmin_{gamma_min}_"
                f"gmax_{gamma_max}_sf_{scale_factor}_epochs_{local_epochs}_frac_{fraction_min}_{fraction_max}_rounds_{num_rounds}.csv")
    ...
    best_model_path = f"best_model_mu_{proximal_mu}_gmin_{gamma_min}_gmax_{gamma_max}_sf_{scale_factor}.pth"
    ...
    for round_num in range(1, num_rounds + 1):
        ...
        for client_idx in selected_clients:
            local_model = initialize_global_model(model_choice, input_dim)
            ...
            if algo_choice == 'fedprox':
                ...
                local_model, loss = train_local_model_fednolowe(local_model, loader, focal_criterion, 
                                                              optimizer, epochs=local_epochs)
                if loss == float('inf') or loss < 1e-4:
                    continue
    ...
    # Final evaluation for this parameter set
    ...
    print(f"\n🎉 Final Results for Parameter Set {param_idx}:")
    print(f"Parameters: proximal_mu={proximal_mu}, gamma_min={gamma_min}, gamma_max={gamma_max}, scale_factor={scale_factor}")
    ...
    # Save predictions for this parameter set
    predictions_file = f"predictions_mu_{proximal_mu}_gmin_{gamma_min}_gmax_{gamma_max}_sf_{scale_factor}.csv"
    save_predictions(global_model, test_loader, predictions_file)
    
    # Save grid search results
    with open(grid_results_file, 'a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([proximal_mu, gamma_min, gamma_max, scale_factor, best_f1, best_balanced_acc, 
                         round_num, final_val_loss, final_f1, final_accuracy, final_balanced_acc, 
                         final_balanced_f1, final_balanced_balanced_acc])
    
    # Update global best
    if best_f1 > best_global_f1:
        best_global_f1 = best_f1
        best_global_balanced_acc = best_balanced_acc
        best_global_params = (proximal_mu, gamma_min, gamma_max, scale_factor)
        torch.save(global_model.state_dict(), best_global_model_path)
        print(f"🌟 New global best Balanced F1: {best_f1:.4f} with params: {best_global_params}")

print(f"\n✅ Grid Search Completed. Best Balanced F1: {best_global_f1:.4f}, Best Balanced Acc: {best_global_balanced_acc:.4f}")
print(f"Best Parameters: proximal_mu={best_global_params[0]}, gamma_min={best_global_params[1]}, "
      f"gamma_max={best_global_params[2]}, scale_factor={best_global_params[3]}")
print(f"Grid search results saved to {grid_results_file}")